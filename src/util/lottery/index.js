var lottery = {
	...
	init: function(id) {
		if ($('#' + id).find('.lottery-unit').length > 0) {
			let _lottery = $('#' + id)
			let _units = _lottery.find('.lottery-unit')
			this.obj = _lottery
			this.count = _units.length
			_lottery.find('.lottery-unit-' + this.index).addClass('active')
		};
	},
	roll: function() {
	...
	stop: function() {
		...
		if (lottery.times > lottery.cycle + 10 && lottery.prize === lottery.index) {
			...
			if (lottery.times < lottery.cycle) {
				...
				lottery.place = Math.random() * (lottery.count) | 0
				// 案例中奖物品通过一个随机数生成
